...
class _DoctorProfileScreenState extends State<DoctorProfileScreen> {
                          ...
                          try {
                            ...
                            if (dialogContext.mounted) {
                              ScaffoldMessenger.of(dialogContext).showSnackBar(
                                const SnackBar(content: Text('Failed to update profile'), backgroundColor: _ProfileColors.error),
                              );
                            }
                          }
                        },
                  style: ElevatedButton.styleFrom(backgroundColor: _ProfileColors.primary),
                  child: isSaving
                      ? const SizedBox(
                          width: 16,
                          height: 16,
      ...
      builder: (context) => AlertDialog(
        ...
        actions: [
          ...
          TextButton(
            ...
            child: const Text('Logout', style: TextStyle(color: _ProfileColors.error)),
          ),
        ],
      ),
    );
  ...
  Widget _buildBody() {
    if (_isLoading) {
      return const Center(child: CircularProgressIndicator(color: _ProfileColors.primary));
    }
      ...
      return Center(
        child: Padding(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              const Icon(Icons.error_outline, color: _ProfileColors.error, size: 40),
              const SizedBox(height: 12),
              Text(_errorMessage ?? 'Profile not found', textAlign: TextAlign.center, style: const TextStyle(color: _ProfileColors.textMuted)),
      ...
      child: ListView(
        ...
        children: [
              ...
              children: [
                CircleAvatar(
                  ...
                  child: Text(
                    ...
                    style: const TextStyle(color: _ProfileColors.primary, fontWeight: FontWeight.bold, fontSize: 28),
                  ),
                ),
                const SizedBox(height: 12),
                Text(profile.name, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                const SizedBox(height: 2),
                Text(profile.specialization, style: const TextStyle(color: _ProfileColors.textMuted, fontSize: 13)),
              ],
            ),
          ),
          ...
          _sectionCard('Professional Details', [
            _infoRow(Icons.medical_services_outlined, 'Specialization', profile.specialization),
            _infoRow(Icons.workspace_premium_outlined, 'License No.', profile.license),
            _infoRow(Icons.apartment_outlined, 'Department', profile.departmentName),
          ]),
          const SizedBox(height: 16),
          ...
          _sectionCard('Consultation Timing', [
            _infoRow(
              ...
              'Available Hours',
              (profile.appointmentStartTime != null && profile.appointmentEndTime != null)
          ...
          SizedBox(
            ...
            child: OutlinedButton.icon(
              ...
              icon: const Icon(Icons.edit_outlined, color: _ProfileColors.primary),
              label: const Text('Edit Profile', style: TextStyle(color: _ProfileColors.primary)),
